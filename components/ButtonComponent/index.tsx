...
export const MainButton = ({ children, type, onClick, className }: Props) => {
    return (
        <button className={`text-sm bg-primary text-white rounded-md py-2 px-4 hover:bg-primary hover:cursor-pointer font-bold ${className}`} type={type} onClick={() => { if (onClick) onClick() }}>
            {children}
        </button>
    )
}
